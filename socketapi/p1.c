/* 
 * A simple peer which reads from stdin and sends it to the peer. Messages received
 * over the network are written to stdout. Mulitple connections are aacepted.
 * Usage:  peer local_port remote_addr remote_port [autoclose]\n"
 */  
	...
	case SCTP_ASSOC_CHANGE:
		...
		if((sac->sac_state == SCTP_COMM_UP) ||
           (sac->sac_state == SCTP_RESTART)) {
            ...
            set_path_parameters(fd, sac->sac_assoc_id, sar, num_rem, HEARTBEAT_TIMEOUT, PATH_MAX_RTX);
 	        sctp_freepaddrs(sar);
