...
int main(int argc, char **argv) 
{
	...
	while (1) {
		...
		if (FD_ISSET(0, &rset)) {
			...
			if (sctp_sendmsg(fd, (void *)buffer, len, (struct sockaddr *)&remote_addr, sizeof(remote_addr), 0, 0, 2, 0, 0) != len)
				perror("sctp_sendmsg");
			else
				printf("Message of length %d sent to %s:%u: %.*s", len, inet_ntoa(remote_addr.sin_addr), ntohs(remote_addr.sin_port), len, buffer);
			if (sctp_sendmsg(fd, (void *)buffer, len, (struct sockaddr *)&remote_addr, sizeof(remote_addr), 0, MSG_ADDR_OVER, 1, 0, 0) != len)
				perror("sctp_sendmsg");
			else
				printf("Message of length %d sent to %s:%u: %.*s", len, inet_ntoa(remote_addr.sin_addr), ntohs(remote_addr.sin_port), len, buffer);
		}
